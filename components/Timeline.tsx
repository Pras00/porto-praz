...
export default function Timeline({ items, type }: TimelineProps) {
  ...
  return (
    <motion.div
      ...
      className="relative pl-6 md:pl-8 border-l border-slate-800 space-y-12 py-2"
    >
      {items.map((item) => (
        <motion.div key={item.id} variants={itemVariants} className="relative group">
          ...
          <div className="absolute -left-7.75 md:-left-9.75 top-1.5 flex items-center justify-center">
            <div className={`w-4 h-4 rounded-full border-2 bg-navy-dark transition-all duration-300 group-hover:scale-125 ${
              isExperience 
          ...
          {/* Timeline Card */}
          <div className="glass-panel hover:border-slate-800/80 rounded-xl p-5 md:p-6 transition-all duration-300 relative overflow-hidden group-hover:translate-x-1">
            ...
            <div className="flex flex-wrap items-start justify-between gap-2 mb-4">
              <div>
                ...
                <p className={`text-sm font-semibold flex items-center gap-1.5 mt-0.5 ${
                  isExperience ? 'text-blue-400' : 'text-purple-400'
                }`}>
                  <Icon className="w-4 h-4 shrink-0" />
                  {item.subtitle}
                </p>
              </div>
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900/60 border border-slate-800 text-xs text-slate-400 font-mono">
                ...
                {item.description.map((desc, i) => (
                  <li key={i} className="flex items-start gap-2.5">
                    <ArrowRight className={`w-3.5 h-3.5 mt-1 shrink-0 ${
                      isExperience ? 'text-blue-500/60' : 'text-purple-500/60'
                    }`} />
                    <span>{desc}</span>
                  </li>
                ))}
